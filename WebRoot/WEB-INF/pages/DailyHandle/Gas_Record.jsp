...
<html>
  <head>
    ...
    $(function(){
    	/*数据表初始设置开始*****************************/  
		$('#rr_dg').datagrid({
            url:'<%=basePath%>refrec_findRRDG.action',
            pagination:true,
            rownumbers:true,
            toolbar: '#rr_tb',
            columns:[[
		...
		$("#unitPrice").blur(function(){ 
				var lname=$.trim($("#unitPrice").val());
		    //将值传送到后台
			if(lname.length==0){
			$.messager.alert("警告","请添油料单价","warning"); 
			...
			$("#this_volu").blur(function(){ 
				var lname=$.trim($("#this_volu").val());
		    //将值传送到后台
			if(lname.length==0){
			$.messager.alert("警告","请输入加油数量","warning"); 
		...
		$('#tb_add').on('click',function(){
			//填充车辆下拉列表
			var url='<%=basePath%>refrec_findCarList.action';
			var carKV={k:'id',v:'carNo'};
			initCombobox($('#rr_addForm #car'),url,carKV,-1);
			
			//填充油气站
			var url='<%=basePath%>refrec_findOilStaList.action';
			var kv={k:'id',v:'unitName'};
			initCombobox($('#rr_addForm #oilSta'),url,kv,-1);
			
			//填充油料标号下拉列表
			var url='<%=basePath%>refrec_findOilLabelList.action';
			...
			//填充加油人员（驾驶员）下拉列表
			var url='<%=basePath%>refrec_findDriverList.action';
    	...
    	$('#rr_addForm #car').combobox({
    		onSelect:function(record){
	    		...
	    		//取得数据
				$.ajax({
					url:'<%=basePath%>refrec_findLastRR.action',
					data:params,
					dataType:'json',
					type:'POST',
					success:function(data){
						//上次里程为查询到的本车牌最近一次加油记录的【本次加油里程】
    	...
    	$('#tb_edit').on('click',function(){
			...
			//取得数据
			$.ajax({
				url:'<%=basePath%>refrec_findRRById.action',
				data:params,
				dataType:'json',
				type:'POST',
				success:function(data){
		...
		$('#tb_remove').on('click',function(){
			...
			$.messager.confirm('确认','您确认想要删除记录吗？',function(r){
				if(r){
					//取得数据
					$.ajax({
						url:'<%=basePath%>refrec_del.action',
						data:params,
						dataType:'json',
						type:'POST',
						success:function(data){
  ...
  <body>
        ...
        buttons:[{
			...
			handler: function(){
				$('#rr_addForm').form('submit',{
					url:'<%=basePath%>refrec_add.action',
					success:function(data){
	...
	<!-- 修改窗体创建位置 -->
	<div id="rr_updDialog" class="easyui-dialog" title="加油信息登记" 
		style="width:400px;height:450px;align:center;"   
        data-options="iconCls:'icon-add',resizable:false,modal:true,closed:true,
        buttons:[{
        	text:'保存',
			...
			handler: function(){
				$('#rr_updForm').form('submit',{
					url:'<%=basePath%>refrec_upd.action',
					success:function(data){
						var obj=$.parseJSON(data);
						if(obj.success){
							//修改成功刷新DG
