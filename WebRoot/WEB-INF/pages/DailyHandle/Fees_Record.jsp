...
<html>
  <head>
    ...
    $(function(){
    	/*数据表初始设置开始*****************************/  
    	$('#fees_dg').datagrid({
            url:'<%=basePath%>fees_findFeesDG.action',
            pagination:true,
            rownumbers:true,            
            toolbar: '#fees_tb',            
            columns:[[
    	...
    	/*数据表初始设置结束*****************************/
    	$("#fname").blur(function(){ 
				var lname=$.trim($("#fname").val());
		    //将值传送到后台
			if(lname.length==0){
			$.messager.alert("警告","请添加规费名称","warning"); 
		...
		$('#tb_add').on('click',function(){
			//填充车辆下拉列表
			var url='<%=basePath%>fees_findCarList.action';
			var carKV={k:'id',v:'carNo'};
			initCombobox($('#fees_addForm #car'),url,carKV,-1);
			
			//填充收费单位下拉列表
			var url='<%=basePath%>fees_findFeesUnitList.action';
			...
			//填充经办人【驾驶员】下拉列表
			var url='<%=basePath%>fees_findDriverList.action';
		...
		$('#tb_edit').on('click',function(){
			...
			//
				$.ajax({
				url:'<%=basePath%>accidentrec_findARById.action',
				data:params,
				dataType:'json',
				type:'POST',
			...
			//取得数据
			$.ajax({
				url:'<%=basePath%>fees_findFeesById.action',
				data:params,
				dataType:'json',
				type:'POST',
				success:function(data){
		...
		<form id="fees_addForm" method="post">
			<table>
				...
				<tr>
					...
					<td>
						<input type="text" name="feesName" id="fname" maxlength="25" required="required"/>
					</td>
				</tr>
				<tr>
