...
<html>
  <head>
	...
	$(function(){
		/*数据表初始设置开始*****************************/  
		$('#pr_dg').datagrid({
            url:'<%=basePath%>peccancyrec_findPRDG.action',
            pagination:true,
            rownumbers:true,
            toolbar: '#pr_tb',
            columns:[[
		...
		$('#tb_add').on('click',function(){
			//填充车辆下拉列表
			var url='<%=basePath%>peccancyrec_findCarList.action';
			var carKV={k:'id',v:'carNo'};
			initCombobox($('#pr_addForm #car'),url,carKV,-1);
			
			//填充违章类别下拉列表
			var url='<%=basePath%>peccancyrec_findOptionList.action';
			...
			//填充驾驶员下拉列表
			var url='<%=basePath%>peccancyrec_findDriverList.action';
		...
		$('#tb_edit').on('click',function(){
			...
			//取得数据
			$.ajax({
				url:'<%=basePath%>peccancyrec_findPRById.action',
				data:params,
				dataType:'json',
				type:'POST',
		...
		$('#tb_remove').on('click',function(){
			...
			$.messager.confirm('确认','您确认想要删除记录吗？',function(r){
				if(r){
					//取得数据
					$.ajax({
						url:'<%=basePath%>peccancyrec_del.action',
						data:params,
						dataType:'json',
						type:'POST',
						success:function(data){
        ...
        buttons:[{
			...
			handler: function(){
				$('#pr_addForm').form('submit',{
					url:'<%=basePath%>peccancyrec_add.action',
					success:function(data){
        ...
        buttons:[{
			...
			handler: function(){
				$('#pr_updForm').form('submit',{
					url:'<%=basePath%>peccancyrec_upd.action',
					success:function(data){
						var obj=$.parseJSON(data);
						if(obj.success){
							//新增成功刷新DG
