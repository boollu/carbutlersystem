...
<html>
  <head>
    ...
    $(function(){
    	/*数据表初始设置开始*****************************/  
		$('#ar_dg').datagrid({
            url:'<%=basePath%>accidentrec_findARDG.action',
            pagination:true,
            rownumbers:true,
            toolbar: '#ar_tb',
            columns:[[
		...
		$('#tb_add').on('click',function(){
			//填充车辆下拉列表
			var url='<%=basePath%>accidentrec_findCarList.action';
			var carKV={k:'id',v:'carNo'};
			initCombobox($('#ar_addForm #car'),url,carKV,-1);
			
			//填充驾驶员下拉列表
			var url='<%=basePath%>accidentrec_findDriverList.action';
		...
		$('#tb_remove').on('click',function(){
			...
			$.messager.confirm('确认','您确认想要删除记录吗？',function(r){
				if(r){
					//取得数据
					$.ajax({
						url:'<%=basePath%>accidentrec_del.action',
						data:params,
						dataType:'json',
						type:'POST',
						success:function(data){
        ...
        buttons:[{
			...
			handler: function(){
				$('#ar_addForm').form('submit',{
					url:'<%=basePath%>accidentrec_add.action',
					success:function(data){
        ...
        buttons:[{
			...
			handler: function(){
				$('#ar_updForm').form('submit',{
					url:'<%=basePath%>accidentrec_upd.action',
					success:function(data){
						var obj=$.parseJSON(data);
						if(obj.success){
							//新增成功刷新DG
