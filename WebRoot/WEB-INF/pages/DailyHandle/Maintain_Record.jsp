...
<html>
  <head>
    ...
    $(function(){
    	/*数据表初始设置开始*****************************/  
		$('#mr_dg').datagrid({
            url:'<%=basePath%>maintainrec_findMRDG.action',
            pagination:true,
            rownumbers:true,
            toolbar: '#mr_tb',
            columns:[[
		...
		$('#tb_add').on('click',function(){
			//填充车辆下拉列表
			var url='<%=basePath%>maintainrec_findCarList.action';
			var carKV={k:'id',v:'carNo'};
			initCombobox($('#mr_addForm #car'),url,carKV,-1);
			
			//填充保养类别下拉列表
			var url='<%=basePath%>maintainrec_findMainTypeList.action';
			initCombobox($('#mr_addForm #mainType'),url,null,-1);
			
			//填充保养单位下拉列表
			var url='<%=basePath%>maintainrec_findMainUnitList.action';
			...
			//填充经办人【驾驶员】下拉列表
			var url='<%=basePath%>maintainrec_findDriverList.action';
    	...
    	/*新增界面选择保养时间计算下次保养时间,结束*****************************/
    	
    	/*新增界面输入本次保养里程计算下次保养里程,开始*****************************/
    	...
    	/*新增界面输入本次保养里程计算下次保养里程,结束*****************************/
    	
    	/*新增界面选择车辆自动更新该车辆对应的下次里程下次保养时间,开始*****************************/
    	...
    	/*新增界面选择车辆自动更新该车辆对应的下次里程下次保养时间,结束*****************************/
		...
		$('#tb_edit').on('click',function(){
			...
			//取得数据
			$.ajax({
				url:'<%=basePath%>maintainrec_findMRById.action',
				data:params,
				dataType:'json',
				type:'POST',
    	...
    	/*修改界面选择保养时间计算下次保养时间,结束*****************************/
    	
    	/*修改界面输入本次保养里程计算下次保养里程,开始*****************************/
    	$('#mr_updForm #mainMil').on('blur',function(){
    		var v1=$('#mr_updForm #mainDate').datebox('getValue');
    		var v2=$('#mr_updForm #nextDate');
    		var v3=$('#mr_updForm #mainMil').numberbox('getValue');
    		var v4=$('#mr_updForm #nextMil');
    		var v5=$('#mr_updForm #car').combobox('getValue');
		...
		$('#tb_remove').on('click',function(){
			...
			$.messager.confirm('确认','您确认想要删除记录吗？',function(r){
				if(r){
					//取得数据
					$.ajax({
						url:'<%=basePath%>maintainrec_del.action',
						data:params,
						dataType:'json',
						type:'POST',
						success:function(data){
    ...
    function autoNextMaintain(dDateValue,$nextDate,nMilValue,$nextMil,nCarId){
    	...
    	//取得数据
		$.ajax({
			url:'<%=basePath%>maintainrec_findCarById.action',
			data:params,
			dataType:'json',
			type:'POST',
			success:function(data){
        ...
        buttons:[{
			...
			handler: function(){
				$('#mr_addForm').form('submit',{
					url:'<%=basePath%>maintainrec_add.action',
					success:function(data){
			...
			<table>
				...
				<tr>
					...
					<td><!-- readonly="readonly" -->
						<input type="text" id="nextDate"  name="nextDate" class="easyui-datebox" />
					</td>
				</tr>
				<tr>
					<td style="text-align: rigth;">下次保养里程（公里）：</td>
					<td>
						<input type="text" id="nextMil" name="nextMil" data-options="min:0" class="easyui-numberbox"/>
					</td>
				</tr>				
				<tr>
        ...
        buttons:[{
			...
			handler: function(){
				$('#mr_updForm').form('submit',{
					url:'<%=basePath%>maintainrec_upd.action',
					success:function(data){
						var obj=$.parseJSON(data);
						if(obj.success){
							//新增成功刷新DG
